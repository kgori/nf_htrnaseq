...
parser$add_argument("--size_factors", help = "Size factors file",
    required = TRUE)
parser$add_argument("--purity_correction", action = "store_true",
    help = "Apply purity correction to the expression values")
...
parser$add_argument("--log_scatterplot_out",
    help = "Output file for log-scaled scatter plots")
...
find_informative_sites <- function(table) { # nolint start
    plot_data <- copy(table[,
        .(Tumour, Chrom, Position, Ref, Alt, is_ctvt_a,
            GeneID, Symbol, nAH, nBH, nAT, nBT, nAHT, nBHT, 
            eA, eA_nrm, eB, eB_nrm, CTVT, HT)])

    # Generate host genotype
    plot_data[, Host := NA_character_]
    plot_data[nAH == 0 & nBH == 0, Host := "-"]
    plot_data[nAH == 1 & nBH == 1, Host := "AB"]
    plot_data[nAH == 2 & nBH == 0, Host := "AA"]
    plot_data[nAH == 0 & nBH == 2, Host := "BB"]
    
    # Generate CTVT genotype
    plot_data[, CTVT := NA_character_]
    plot_data[nAT == 0 & nBT == 0, CTVT := "-"]
    plot_data[nAT == 1 & nBT == 0, CTVT := "A"]
    plot_data[nAT == 0 & nBT == 1, CTVT := "B"]    
    plot_data[nAT >= 1 & nBT == 0, CTVT := "AA"]
    plot_data[nAT == 0 & nBT >= 1, CTVT := "BB"]
    plot_data[nAT >= 1 & nBT >= 1, CTVT := "AB"]
    
    # Generate HT genotype
    plot_data[, HT := NA_character_]
    plot_data[nAHT == 0 & nBHT == 0, HT := "-"]
    plot_data[nAHT == 1 & nBHT == 0, HT := "A"]
    plot_data[nAHT == 0 & nBHT == 1, HT := "B"]
    ...
    plot_data[HT == "A" & CTVT ==  "-" & Host == "BB", c("ht_only", "allele_ht_only") := .(TRUE, "A")]
    plot_data[HT == "B" & CTVT ==  "-" & Host == "AA", c("ht_only", "allele_ht_only") := .(TRUE, "B")]
    ...
    plot_data[HT == "A" & CTVT ==  "-" & Host == "AB", c("host", "allele_host") := .(TRUE, "B")]
    plot_data[HT == "A" & CTVT ==  "-" & Host == "BB", c("host", "allele_host") := .(TRUE, "B")]
    plot_data[HT == "B" & CTVT ==  "-" & Host == "AA", c("host", "allele_host") := .(TRUE, "A")]
    plot_data[HT == "B" & CTVT ==  "-" & Host == "AB", c("host", "allele_host") := .(TRUE, "A")]
    plot_data
} # nolint end
...
summarise_informative_alleles <- function(plot_data) { # nolint start
    ...
    setkey(plot_data, Tumour, Chrom, Position, Ref, Alt, is_ctvt_a, GeneID, Symbol, HT, CTVT, Host, eA_nrm, eB_nrm)
    setkey(melttwice, Tumour, Chrom, Position, Ref, Alt, is_ctvt_a, GeneID, Symbol, HT, CTVT, Host, eA_nrm, eB_nrm)
    melttwice <- plot_data[melttwice, nomatch = NULL]

    melttwice[, informative_allele_count := 0]
    melttwice[
        plot_category == "host",
        informative_allele_count := ifelse(informative_allele == "A", nAH, nBH)
    ]
    melttwice[
        plot_category == "ctvt",
        informative_allele_count := ifelse(informative_allele == "A", nAT, nBT)
    ]
    melttwice[
        plot_category == "ht_only",
        informative_allele_count := ifelse(informative_allele == "A", nAHT, nBHT)
    ]
    melttwice[
        plot_category == "ht_and_ctvt",
        informative_allele_count := ifelse(informative_allele == "A", nAT + nAHT, nBT + nBHT)
    ]
...
if (args$purity_correction) {
    selected_plot_data[deconv_table, purity := purity, on = "Tumour"]
    selected_plot_data[, expression := expression / ifelse(plot_category == "host", 1 - purity, purity)]
}
...
build_gene_stats_table <- function(gene, data, category = c("tumour", "host", "ht"), take_log = FALSE) {
    category <- match.arg(category)
    gene_data <- data[Symbol == gene]
    if (take_log) {
        gene_data[, expression := log(expression + 1)]
    }
...
log_gene_stats <- rbindlist(lapply(genes, function(gene) {
    rbindlist(lapply(c("tumour", "host", "ht"), function(cat) {
        build_gene_stats_table(gene, sample_mean_plot_data, cat, take_log = TRUE)
    }))
}))
...
scatterplot <- function(data, xsample = "tumour", ysample = "ht") {
    ...
    xy_all <- dcast(xy_data, gene ~ category, value.var = c("mean", "ci95_l", "ci95_u"))
    ...
    if (!("(Intercept)" %in% names(coeffs))) {
        coeffs <- append(c("(Intercept)" = 0), coeffs)
    }
    rsq <- summary(fit)$r.squared
    correlation <- xy_mean[cc, cor(get(ysample), get(xsample))]
    correlation_pval <- xy_mean[cc, cor.test(get(ysample), get(xsample))$p.value]
    print(correlation_pval)
    print(summary(fit))
    print(coeffs)
    do.call(abline, append(list(coeffs),
                           list(col = "grey80")))
    ...
    error_bars <- xy_all[, .(x0 = get(paste("ci95_l", xsample, sep = "_")),
               x1 = get(paste("ci95_u", xsample, sep = "_")),
               x = get(paste("mean", xsample, sep = "_")),
               y = get(paste("mean", ysample, sep = "_")),
               y0 = get(paste("ci95_l", ysample, sep = "_")),
               y1 = get(paste("ci95_u", ysample, sep = "_")))]

    error_bars[, segments(x0 = x0, y0 = y, x1 = x1, y1 = y)]
    error_bars[, segments(x0 = x, y0 = y0, x1 = x, y1 = y1)]
    
    legend("topright",
        legend = c(sprintf("r^2 = %.3f", rsq),
            sprintf("r = %.3f p = %.3f", correlation, correlation_pval)),
        bty = "n")
}
...
pdf(args$log_scatterplot_out, height = 8.25 / 2, width = 8.25 / 2)
scatterplot(log_gene_stats, "tumour", "ht")
scatterplot(log_gene_stats, "host", "ht")
scatterplot(log_gene_stats, "host", "tumour")
dev.off()
